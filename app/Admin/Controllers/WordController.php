...
use App\Admin\Actions\Word\LikeWord;
...
class WordController extends Controller
{
    ...
    protected function grid()
    {
        ...
        $grid->selector(function (Grid\Tools\Selector $selector) {
            ...
            $selector->select('tags', 'Tags',  Tag::where('group', Word::NAME)->pluck('name', 'id'), function ($query, $value) {
                $word_ids = [];
                foreach ($value as $id) {
                    $tmp_words = DB::table("word_tag")->where('tag_id', $id)->get();
                    foreach ($tmp_words as $tmp_word) {
                        array_push($word_ids, $tmp_word->word_id);
                    }
                }
                $query->whereIn('id', $word_ids);
            });
        });
        ...
        $grid->filter(function (Grid\Filter $filter) {
            ...
            $filter->column(1 / 3, function ($filter) {
                $filter->equal('type', 'Type')->select(function () {
                    $query = Type::where('group', Word::NAME)->pluck('name', 'id');
                    return $query;
                });
            });
            $filter->column(1 / 3, function ($filter) {
        ...
        $grid->quickCreate(function (Grid\Tools\QuickCreate $create) {
            ...
            $tags = Tag::where('group', Word::NAME)->pluck('name', 'id');
    ...
    protected function form()
    {
        ...
        $types = Type::where('group', Word::NAME)->pluck('name', 'id');
